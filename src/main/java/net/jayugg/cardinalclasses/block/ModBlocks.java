...
public class ModBlocks {
    ...
    public static final Block SKILL_ALTAR = registerBlock("skill_altar",
            new SkillAltarBlock(FabricBlockSettings.of(Material.STONE, MapColor.DEEPSLATE_GRAY).requiresTool().strength(50.0f, 1200.0f).sounds(BlockSoundGroup.DEEPSLATE)), ModItemGroup.MOD_GROUP);
    ...
    private static Item registerBlockItem(String name, Block block, ItemGroup group) {
        Item blockItem = Registry.register(Registries.ITEM, new Identifier(MOD_ID, name), new BlockItem(block, new Item.Settings()));
        ItemGroupEvents.modifyEntriesEvent(group).register(entries -> entries.add(blockItem));
        return blockItem;
    }
    public static void registerModBlocks() {
        CardinalClasses.LOGGER.debug("Registering Mod Blocks for " + MOD_ID);
        // register item group after blocks have been registered
    }
}
