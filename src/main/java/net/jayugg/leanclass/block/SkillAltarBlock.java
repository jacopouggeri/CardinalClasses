...
public class SkillAltarBlock extends BlockWithEntity {
    ...
    public static void loadChargeItem(@Nullable Entity charger, World world, BlockPos pos, BlockState state, ItemStack stack) {
        ...
        if (charge == AltarCharge.ASCEND) { return; }
...
private static void updateChargeItem(@Nullable Entity charger, World world, BlockPos pos, BlockState blockState) {
    ...
    private PerkSlot offsetToPerkSlot(BlockPos offset) {
        LeanClass.LOGGER.warn("Offset: {}", offset);
        LeanClass.LOGGER.warn(String.valueOf((int) (((float)offset.getX() + offset.getZ())/4 + 1.5f)));
        LeanClass.LOGGER.warn(PerkSlot.fromInt((int) (((float)offset.getX() + offset.getZ())/4 + 1.5f)).toString());
